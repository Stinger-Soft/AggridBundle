...
class SetFilterType extends AbstractFilterType
{
    ...
    public function configureOptions(OptionsResolver $resolver, array $columnOptions = [], array $gridOptions = []): void
    {
        ...
        $resolver->setDefault('comparator', null);
        $resolver->setAllowedTypes('comparator', ['null', 'string']);
    ...
    public function buildJsonConfiguration(FilterView $view, FilterInterface $filter, array $options): void
    {
        ...
        $view->jsonConfiguration['filterParams'] ??= [];
        if ($view->vars['data']) {
     ...
     * @param mixed $value
     * @param mixed $toValue
     * @return \Doctrine\ORM\Query\Expr|\Doctrine\ORM\Query\Expr\Comparison|\Doctrine\ORM\Query\Expr\Andx|\Doctrine\ORM\Query\Expr\Orx|null
     */
    ...
    public function buildView(FilterView $view, FilterInterface $filter, array $options, $dataSource, string $queryPath, string $rootAlias): void
    {
        ...
        } elseif (is_callable($options['data'])) {
