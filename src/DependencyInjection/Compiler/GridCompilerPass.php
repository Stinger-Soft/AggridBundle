...
class GridCompilerPass implements CompilerPassInterface
{
    ...
    /** @var string */
    protected $filterTypeExtensionTag;
    ...
    protected function processExtensionType(ContainerBuilder $container, string $tagType): array
    {
        ...
        foreach ($this->findAndSortTaggedServices($tagType, $container) as $reference) {
            ...
            if (!method_exists($typeExtensionClass, 'getExtendedTypes')) {
                throw new InvalidArgumentException(sprintf('"%s" tagged services have to implement the static getExtendedTypes() method. Class "%s" for service "%s" does not implement it.', $tagType, $typeExtensionClass, $serviceId));
            }
            $extendsTypes = false;
    ...
    /**
     * @param array<string, Reference> $servicesMap
